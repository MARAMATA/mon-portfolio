...
const AdminDashboard = () => {
  ...
  const fetchProjects = useCallback(() => {
    setLoading(true);
    axios
      .get("http://localhost:5001/api/admin/projects", {
  ...
  const fetchContacts = useCallback(() => {
    axios
      .get("http://localhost:5001/api/admin/contacts", {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  }, [token, fetchProjects, fetchContacts]);
  ...
  const handleAddProject = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:5001/api/projects/", newProject, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
  ...
  const handleDeleteProject = (projectId) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce projet ?")) {
      axios
        .delete(`http://localhost:5001/api/projects/${projectId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then(() => {
